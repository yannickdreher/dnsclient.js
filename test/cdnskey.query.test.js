...
describe('Query type "CDNSKEY" should return the correct data', () => {
    let result;
    const question = new dnsclient.Question('dremaxx.de', dnsclient.TYPE.CDNSKEY, dnsclient.CLAZZ.IN);
    ...
    test('Record type of answers is "CDNSKEY"', () => {
        result.message.answers.forEach(answer => {
            expect(answer.type).toBe('CDNSKEY');
        });
    });
